...
mixin TableHelper {
  ...
  static Table fromDataArray<T>({
    Context? context,
    required List<T> data,
    required TableRow Function(T data) rowBuilder,
    required T headers,
    required TableRow Function(T data) headerBuilder,
    EdgeInsetsGeometry cellPadding = const EdgeInsets.all(5),
    double cellHeight = 0,
    AlignmentGeometry cellAlignment = Alignment.topLeft,
    Map<int, AlignmentGeometry>? cellAlignments,
    ...
    for (final row in data) {
      ...
      // final isOdd = (rowNum - headerCount) % 2 != 0;
      // if (rowNum < headerCount) {
      //   // tableRow.addAll(rowBuilder(row));
      //   // for (final dynamic cell in row) {
      //   //   final align = headerAlignments[tableRow.length] ?? headerAlignment;
      //   //   final textAlign = _textAlign(align.resolve(textDirection));
      ...
      final b0 = rowBuilder(row);

      rows.add(TableRow(
        children: b0.children,
        repeat: rowNum < headerCount,
        decoration: b0.decoration,
      ));
      rowNum++;
    }
    return Table(
  ...
  static Widget fromGroupDataArray<T>({
    Context? context,
    required List<T> data,
    required TableRow Function(T data) rowBuilder,
    T? headers,
    required TableRow Function(T data) headerBuilder,
    required Widget? groupTitle,
    EdgeInsetsGeometry cellPadding = const EdgeInsets.all(5),
    double cellHeight = 0,
    AlignmentGeometry cellAlignment = Alignment.topLeft,
    Map<int, AlignmentGeometry>? cellAlignments,
