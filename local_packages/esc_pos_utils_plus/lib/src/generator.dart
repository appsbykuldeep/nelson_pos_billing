...
class Generator {
  ...
  int get maxCharsPerLineFontA => _getMaxCharsPerLine(PosFontType.fontA);
  int get maxCharsPerLineFontB => _getMaxCharsPerLine(PosFontType.fontB);
  ...
  List<List<int>> _toColumnFormat(Image imgSrc, int lineHeight) {
    ...
    fill(biggerImage, Color.fromRgb(0, 0, 0));
  ...
  List<int> cut({PosCutMode mode = PosCutMode.full, int afterLines = 3}) {
    List<int> bytes = [];
    bytes += emptyLines(afterLines);
