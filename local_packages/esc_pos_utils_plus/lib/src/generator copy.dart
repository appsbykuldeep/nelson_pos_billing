...
class Generator {
  ...
  List<List<int>> _toColumnFormat(Image imgSrc, int lineHeight) {
    ...
    biggerImage = fill(biggerImage, img.Color.fromRgb(0, 0, 0));
    // Insert source image into bigger one
    // compositeImage(biggerImage, image, dstX: 0, dstY: 0);
    biggerImage = drawImage(biggerImage, image, dstX: 0, dstY: 0);
    ...
    while (left < widthPx) {
      ...
      // grayscale(slice);
      // if (slice.numberOfChannels > 2) {
      //   grayscale(slice);
      // }
      // final imgBinary =
      //     (slice.numberOfChannels > 1) ? slice.convert(numChannels: 1) : slice;
  ...
  List<int> cut({PosCutMode mode = PosCutMode.full, int afterLines = 2}) {
    List<int> bytes = [];
