...
countryCodes = [
  ...
  },
  {
    ...
    "full_example_with_plus_sign": None,
...
ct_mlc = {'AF':'ps','AX':'sv','AL':'sq','DZ':'ar','AS':'en','AD':'ca','AO':'pt','AI':'en','AG':'en','AR':'es','AM':'hy','AW':'nl','AC':'en','AU':'en','AT':'de','AZ':'az','BS':'en','BH':'ar','BD':'bn','BB':'en','BY':'be','BE':'nl','BZ':'es','BJ':'fr','BM':'en','BT':'dz','BO':'es','BA':'bs','BW':'en','BR':'pt','IO':'en','VG':'en','BN':'ms','BG':'bg','BF':'fr','BI':'fr','KH':'km','CM':'en','CA':'en','CV':'pt','BQ':'nl','KY':'en','CF':'fr','TD':'fr','CL':'es','CN':'zh','CX':'en','CC':'en','CO':'es','KM':'ar','CD':'fr','CG':'fr','CK':'en','CR':'es','CI':'fr','HR':'hr','CU':'es','CW':'nl','CY':'el','CZ':'cs','DK':'da','DJ':'ar','DM':'en','DO':'es','TL':'tet','EC':'es','EG':'ar','SV':'es','GQ':'es','ER':'ti','EE':'et','SZ':'si','ET':'am','FK':'en','FO':'fo','FJ':'en','FI':'fi','FR':'fr','GF':'fr','PF':'fr','GA':'fr','GM':'en','GE':'ka','DE':'de','GH':'en','GI':'en','GR':'el','GL':'kl','GD':'en','GP':'fr','GU':'en','GT':'es','GG':'en','GN':'fr','GW':'pt','GY':'en','HT':'ht','HM':'en','HN':'es','HK':'zh','HU':'hu','IS':'is','IN':'hi','ID':'id','IR':'fa','IQ':'ar','IE':'en','IM':'en','IL':'he','IT':'it','JM':'en','JP':'ja','JE':'en','JO':'ar','KZ':'kk','KE':'en','KI':'en','XK':'sq','KW':'ar','KG':'ky','LA':'lo','LV':'lv','LB':'ar','LS':'st','LR':'en','LY':'ar','LI':'de','LT':'lt','LU':'lb','MO':'zh','MK':'mk','MG':'mg','MW':'ny','MY':'ms','MV':'dv','ML':'bm','MT':'mt','MH':'en','MQ':'fr','MR':'ar','MU':'en','YT':'fr','MX':'es','FM':'en','MD':'ro','MC':'fr','MN':'mn','ME':'sr','MS':'en','MA':'ar','MZ':'pt','MM':'my','NA':'en','NR':'en','NP':'ne','NL':'nl','NC':'fr','NZ':'en','NI':'es','NE':'fr','NG':'en','NU':'en','NF':'en','KP':'ko','MP':'en','NO':'no','OM':'ar','PW':'en','PS':'ar','PA':'es','PG':'en','PY':'es','PE':'es','PH':'fil','PL':'pl','PT':'pt','PR':'es','QA':'ar','RE':'fr','RO':'ro','RU':'ru','RW':'rw','BL':'fr','SH':'en','KN':'en','LC':'en','MF':'fr','PM':'fr','VC':'en','WS':'sm','SM':'it','ST':'pt','SA':'ar','SN':'fr','RS':'sr','SC':'en','SL':'en','SG':'en','SX':'nl','SK':'sk','SI':'sl','SB':'en','SO':'so','ZA':'en','GS':'en','KR':'ko','SS':'en','ES':'es','LK':'si','SD':'ar','SR':'nl','SJ':'no','SE':'sv','CH':'de','SY':'ar','TW':'zh-TW','TJ':'tg','TZ':'sw','TH':'th','TG':'fr','TK':'en','TO':'to','TT':'en','TN':'ar','TR':'tr','TM':'tk','TC':'en','TV':'en','VI':'en','UG':'en','UA':'uk','AE':'ar','GB':'en','US':'en','UY':'es','UZ':'uz','VU':'en','VA':'it','VE':'es','VN':'vi','WF':'fr','EH':'ar','YE':'ar','ZM':'en','ZW':'en'}

curr_symbol = {
    
    'AF':'؋','AX':'€','AL':'L','DZ':'د.ج','AS':'\$','AD':'€','AO':'Kz','AI':'\$','AG':'\$','AR':'\$','AM':'֏','AW':'ƒ','AC':'£','AU':'\$','AT':'€','AZ':'₼','BS':'\$','BH':'.د.ب','BD':'৳','BB':'\$','BY':'Br','BE':'€','BZ':'BZ\$','BJ':'CFA','BM':'\$','BT':'Nu.','BO':'Bs.','BA':'KM','BW':'P','BR':'R\$','IO':'£','VG':'\$','BN':'B\$','BG':'лв','BF':'CFA','BI':'FBu','KH':'៛','CM':'CFA','CA':'\$','CV':'CV\$','BQ':'€','KY':'CI\$','CF':'CFA','TD':'CFA','CL':'\$','CN':'¥','CX':'\$','CC':'\$','CO':'\$','KM':'KMF','CD':'CDF','CG':'CFA','CK':'\$','CR':'₡','CI':'CFA','HR':'kn','CU':'₱','CW':'NAƒ','CY':'€','CZ':'Kč','DK':'kr','DJ':'Fdj','DM':'\$','DO':'RD\$','TL':'\$','EC':'\$','EG':'ج.م','SV':'\$','GQ':'XAF','ER':'Nfa','EE':'€','SZ':'E','ET':'Br','FK':'£','FO':'kr','FJ':'FJ\$','FI':'€','FR':'€','GF':'€','PF':'CFP','GA':'CFA','GM':'D','GE':'₾','DE':'€','GH':'GH₵','GI':'£','GR':'€','GL':'kr','GD':'\$','GP':'€','GU':'\$','GT':'Q','GG':'£','GN':'FG','GW':'CFA','GY':'G\$','HT':'G','HM':'\$','HN':'L','HK':'HK\$','HU':'Ft','IS':'kr','IN':'₹','ID':'Rp','IR':'﷼','IQ':'ع.د','IE':'€','IM':'£','IL':'₪','IT':'€','JM':'J\$','JP':'¥','JE':'£','JO':'د.ا','KZ':'₸','KE':'KSh','KI':'\$','XK':'€','KW':'د.ك','KG':'сом','LA':'₭','LV':'€','LB':'ل.ل','LS':'L','LR':'L','LY':'ل.د','LI':'CHF','LT':'€','LU':'€','MO':'MOP','MK':'MKD','MG':'MGA','MW':'MK',
...
def get_main_language_code(country_code):
    return ct_mlc.get(country_code)

def get_Currency_symbol(country_code):
    sym =  curr_symbol.get(country_code)
    if sym is None:
        return None
    if sym.startswith("\\") and sym.startswith("\\u") == False :
...
#         if not country:
#             return None
#         # Use langcodes to get the most likely language for the country
#         language = langcodes.closest_langcode_for_region(country.alpha_2)
#         return language.language  # Returns 'en', 'fr', etc.
...
def get_country_details(country_code):
    ...
    currency_code = currency.alpha_3 if currency else 'Unknown'
    ...
    for tz in timezones:
        ...
        # print("offset",offset)
        utc_offsets.append(f"{tz} (UTC{offset[:3]}:{offset[3:]})")
        utc_timediff = f"{offset[:3]}:{offset[3:]}"
        break

    return {
        'country': country.name,
        'currency': currency_code,
        'timezone': utc_timediff
    }

output = []

for x in countryCodes:
    countryCode = x["iso2_cc"] 
    tt = get_country_details(countryCode)
    # print(tt)
    mlg = get_main_language_code(countryCode)
    ...
    "name": x["name"],
    # "mainLang": mlg,
    "currencySymbol": get_Currency_symbol(countryCode),
    "timezone": tt.get("timezone"),
    "mobMinLen": len(x.get("example","")),
  })
    # break
